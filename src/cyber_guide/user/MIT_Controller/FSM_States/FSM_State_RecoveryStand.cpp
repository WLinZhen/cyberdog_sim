...
bool FSM_State_RecoveryStand<T>::_UpsideDown(){
  ...
  if(fabs(this->_data->_stateEstimator->getResult().rpy(0)) > 1.0f){
    return true;
  }
  return false;
}
...
void FSM_State_RecoveryStand<T>::_RollOver(const int & curr_iter){
  if(this->_data->_stateEstimator->getResult().rpy(0) > 1.0){
    rolling_jpos[0] << 1.5f, -1.6f, 2.77f;
    rolling_jpos[1] << 1.3f, -3.1f, 2.77f;
    rolling_jpos[2] << 1.5f, -1.6f, 2.77f;
    rolling_jpos[3] << 1.3f, -3.1f, 2.77f;
  }else{
    rolling_jpos[1] << 1.5f, -1.6f, 2.77f;
    rolling_jpos[0] << 1.3f, -3.1f, 2.77f;
    rolling_jpos[3] << 1.5f, -1.6f, 2.77f;
    rolling_jpos[2] << 1.3f, -3.1f, 2.77f;
  }
...
void FSM_State_RecoveryStand<T>::_StandUp(const int & curr_iter){
  ...
  if( _UpsideDown() || (body_height < 0.05 ) ) {
